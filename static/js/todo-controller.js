        ...
        $http.post(url, data).then(
            ...
            (resp) => { 
                ...
                if (resp.status === 304){
                    _sendGetUpdateAngModel();
                    $scope.newTaskName = '';        
                    $scope.addAlert('This task is already created, you can not create dublicate task' );
                }
            });
    };
    ...
    $scope.onClickDeleteTask = (taskID, taskName) => {
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {  // Members that will be resolved and passed to the modal-controller as locals
                name: () => { // name should be consistent with argument in modal-controller
                    return taskName;
                }
            }
        });
        ...
        modalInstance.result.then((modalResult) => {
            // resolve function, modalResult passed from modal-controller
            ...
            // reject function, modalResultReject passed from modal-controller
